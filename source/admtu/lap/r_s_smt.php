...
if (empty($tapelkd))
	...
	if ($count != 0)
		...
		do
			...
			echo '<td align="center"><input name="SW'.$nomer.'" type="hidden" value="'.$kd.'">'.nosql($data['no_absen']).'</td>
			<td align="center">'.nosql($data['nis']).'</td>
			<td>'.balikin2($data['nama']).'</td>
			<td>[<a href="';
			$redir = "r_s_smt$jns";
			$redir1 = "_pdf";
			echo "$redir$redir1.php?swkd=$kd&kelkd=$kelkd&progkd=$progkd&rukd=$rukd&tapelkd=$tapelkd&smtkd=$smtkd";
